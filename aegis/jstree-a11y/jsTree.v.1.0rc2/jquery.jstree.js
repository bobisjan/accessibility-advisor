...
//			var container = this.data.html_data.original_container_html;
			var container = '.summary.tree';
			
			$(container).find("a").attr("tabindex", "-1").eq(0).attr("tabindex", "0");
			$.each(this._get_settings().hotkeys, function (i, val) {
				if($.inArray(i, bound) == -1) {
				    $(container).bind("keydown", i, function (event) { return exec(i, event); });
					bound.push(i);
				}
			});
			this.enable_hotkeys();
